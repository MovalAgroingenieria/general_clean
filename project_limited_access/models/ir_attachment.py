...
class Attachment(models.Model):
    ...
    def check(self, mode, values=None):
        ...
        if self.env.is_superuser():
            return True
        # Always require an internal user (aka, employee) to access to an
        # attachment (This modification is for this modele)
        ...
        for res_model, res_ids in model_ids.items():
            ...
            if res_model not in self.env:
                continue
            if res_model == 'res.users' and len(res_ids) == 1 and \
                    self.env.uid == list(res_ids)[0]:
