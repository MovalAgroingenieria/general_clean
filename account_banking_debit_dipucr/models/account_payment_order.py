...
class AccountPaymentOrder(models.Model):
    ...
    entity = fields.Char(
        string="Entity",
        default=_get_entity_config_dipucr,
    ...
    def _compute_payment_mode_name(self):
        # @INFO: payment mode name DipuCR (mandatory)
        for record in self:
    ...
    def _onchange_payment_mode_name(self):
        if self.payment_mode_name == 'DipuCR':
    ...
    def generate_payment_file(self):
        ...
        if pay_method.code != 'debit_dipucr':
        ...
        for line in self.bank_line_ids:
            ...
            # Check if bank line is already done by dipucr
            if line.dipucr_sent:
                if self.error_mode == 'permissive':
                    ...
                    errors += '[' + str(error_num).zfill(4) + '] ' + \
                        _("The entry number %s has failed, the bank line "
                          ...
                          "DipuCR" % (entry_num_padded, line.name)) + '\n'
                else:
                    raise ValidationError(
                        _("The entry number %s has failed, the bank line "
                          "%s seems that it has already been sent through "
            ...
            # Set as done and add DipuCR list ref
            line.write({
                'dipucr_ref': value_list_refnum,
                'dipucr_sent': True,
                })
    ...
    # This method is replicated by wua_account_banking_debit_dipucr
    @api.multi
