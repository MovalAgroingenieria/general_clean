...
class HrAttendance(models.Model):
    ...
    def onchange_checkin_checkout(self):
        ...
        for record in self:
            ...
            current_att = record.env['hr.attendance'].browse(origin_id)
            current_time = datetime.now()
            time_limit_down = current_time - timedelta(minutes=30)
            time_limit_up = current_time + timedelta(minutes=30)
            check_in = False
            check_out = False
            current_att_check_in = False
            current_att_check_out = False
            if record.check_in:
                check_in = datetime.strptime(
                    record.check_in, '%Y-%m-%d %H:%M:%S')
            if record.check_out:
                check_out = datetime.strptime(
                    record.check_out, '%Y-%m-%d %H:%M:%S')
            if current_att.check_in:
                current_att_check_in = datetime.strptime(
                    current_att.check_in, '%Y-%m-%d %H:%M:%S')
            if current_att.check_out:
                current_att_check_out = datetime.strptime(
                    current_att.check_out, '%Y-%m-%d %H:%M:%S')
            # Detect if attendance is modified and edited or is
            # created outside the current time range
            attendance_modificated = False
            if current_att.id and check_in:
                if current_att_check_in != check_in:
                    current_att.write({'attendance_edited': True})
                if current_att_check_in != check_in and not att_admin:
                    attendance_modificated = True
                    record.attendance_modificated = attendance_modificated
                    if (check_in < time_limit_down or
                            check_in > time_limit_up) and not att_admin:
                        record.attendance_modificated = True
                    else:
                        record.attendance_modificated = False
            if check_out and current_att_check_out:
                if current_att_check_out != check_out:
                    current_att.write({'attendance_edited': True})
                if current_att_check_out != check_out and not att_admin:
                    attendance_modificated = True
                    record.attendance_modificated = attendance_modificated
            if (check_out and current_att_check_out != check_out):
                if (check_out < time_limit_down or
                        check_out > time_limit_up) and not att_admin:
                    record.attendance_modificated = True
                else:
                    record.attendance_modificated = False
